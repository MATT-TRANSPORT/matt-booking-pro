...
export async function POST(req: NextRequest) {
  ...
  const { data: membership } = await auth
    .from("company_users")
    .select("company_id,role,companies(email,name,nip,default_payment_method)")
    .eq("user_id", user.id)
    .eq("active", true)
    .single();
  ...
  if (body.employeeId) {
    ...
    if (!employee && newEmployee.phone) {
      const { data } = await admin
        .from("company_employees")
        .select("*")
        .eq("company_id", membership.company_id)
        .eq("phone", newEmployee.phone)
        .limit(1)
        .maybeSingle();
      employee = data;
    }
    ...
    if (!employee) {
      const { data, error } = await admin
        ...
        .insert({
          ...
          default_address: newEmployee.defaultAddress || body.address || null,
  ...
  let quote;
  try {
    // Cena zawsze liczona ponownie na backendzie.
    // distanceKm / total przesłane z przeglądarki są ignorowane.
  ...
  const paymentMethod =
    body.paymentMethod === "employee_payment"
      ...
      ? "company_transfer"
      : quote.defaultPaymentMethod === "employee_payment"
      ? "employee_payment"
      : "company_transfer";
  ...
  await admin.from("booking_history").insert({
    booking_id: data.id,
    event:
      `Rezerwacja utworzona przez portal B2B PRO · ` +
      `${quote.net.toFixed(2)} zł netto + VAT ${quote.vat.toFixed(2)} zł = ${quote.gross.toFixed(2)} zł brutto · ` +
      `${quote.distanceFromHeadquartersKm.toFixed(1)} km od siedziby, ${quote.billableKm.toFixed(1)} km płatne`,
    created_by: user.id
  });
  ...
  try {
    const companyAdminEmail = company?.email || user.email || null;
    ...
    const adminResult = await sendMattEmail({
      ...
      subject: `B2B PRO · ${adminMail.subject}`,
      html: adminMail.html
    });
