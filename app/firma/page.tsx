...
export default async function Page() {
  ...
  const { data } = await s.from("bookings").select("*").eq("company_id", company.id).order("travel_date", { ascending: true }).limit(500);
  ...
  const bookingCard = (b: any, archived = false) => {
    ...
    return <article className={`company-booking-card${archived ? " company-booking-card-archive" : ""}`} key={b.id}>
      <div className="company-card-head"><a href={`/firma/rezerwacje/${b.id}`}><strong>{b.booking_number}</strong></a><span className={`status ${String(b.status).toLowerCase()}`}>{statusPl(b.status)}</span></div>
  ...
  return <main className="container">
    ...
    <div className="card company-dashboard-transport-card">
      ...
      {!rows.length ? <div className="company-dashboard-empty"><strong>Brak zamówionych transportów.</strong><span>Nową rezerwację możesz utworzyć przyciskiem powyżej.</span></div> : <>
        <div className="company-bookings-table"><table className="table company-table"><thead><tr><th>Numer</th><th>Termin</th><th>Pasażer</th><th>Trasa</th><th>Netto</th><th>Płatność</th><th>Status</th></tr></thead><tbody>
          {activeRows.map((b:any)=>bookingRow(b))}
          {archiveRows.length>0&&<tr className="company-booking-archive-divider"><td colSpan={7}>HISTORIA · ZAKOŃCZONE / ANULOWANE / TERMIN MINĄŁ</td></tr>}
