...
export default async function Page({searchParams}:{searchParams:Promise<{q?:string}>}){
  ...
  let query=s.from("bookings").select("*").order("created_at",{ascending:false}).limit(300);
  ...
  return <main className="container"><h1>Archiwum rezerwacji</h1><PanelNav/>
    ...
    <div className="desktop-table card"><table className="table"><thead><tr><th>Numer</th><th>Termin</th><th>Klient</th><th>Cena</th><th>Status</th></tr></thead><tbody>
      {(data??[]).map((b:any)=><tr key={b.id}><td><a className="booking-number-link" href={`/panel/rezerwacje/${b.id}`}>{b.booking_number}</a></td><td>{b.travel_date}<br/>{b.travel_time}</td><td>{b.customer_name}<br/>{b.phone}</td><td>{Number(b.total_price).toFixed(2)} zł</td><td>{statusPl(b.status)}</td></tr>)}
    </tbody></table></div>
