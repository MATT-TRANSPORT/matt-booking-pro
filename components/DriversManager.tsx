...
export default function DriversManager({drivers}:{drivers:any[]}) {
  ...
  const empty={id:"",fullName:"",phone:"",email:"",licenseNumber:"",userId:"",notes:"",color:"#D6AD55",costPerTrip:"",active:true};
  const [form,setForm]=useState<any>(empty);
  const [message,setMessage]=useState("");
  const [saving,setSaving]=useState(false);

  function edit(x:any){
    setForm({id:x.id,fullName:x.full_name||"",phone:x.phone||"",email:x.email||"",licenseNumber:x.license_number||"",userId:x.user_id||"",notes:x.notes||"",color:x.color||"#D6AD55",costPerTrip:x.cost_per_trip??"",active:x.active!==false});
  ...
  return <>
    <div className="card">
      ...
      <div className="grid">
        ...
        <label>Kolor kierowcy<input type="color" value={form.color} onChange={e=>setForm({...form,color:e.target.value})}/></label>
        <label>Szac. koszt kierowcy / kurs (zł)<input type="number" min={0} step="0.01" value={form.costPerTrip} onChange={e=>setForm({...form,costPerTrip:e.target.value})} placeholder="opcjonalnie"/></label>
        <label>UUID konta logowania<input value={form.userId} onChange={e=>setForm({...form,userId:e.target.value})} placeholder="Supabase Auth user UUID"/></label>
      </div>
      <p className="muted">Koszt / kurs jest opcjonalny i służy wyłącznie do szacunkowego raportu rentowności.</p>
    ...
    <div className="card" style={{marginTop:18}}>
      <h2>Kierowcy</h2>
      <table className="table"><thead><tr><th>Kierowca</th><th>Telefon</th><th>E-mail</th><th>Koszt/kurs</th><th>Status</th><th></th></tr></thead>
      <tbody>{drivers.map((x:any)=><tr key={x.id}><td><span className="driver-color-dot" style={{background:x.color||"#D6AD55"}}/><strong>{x.full_name}</strong></td><td>{x.phone||"—"}</td><td>{x.email||"—"}</td><td>{x.cost_per_trip!==null&&x.cost_per_trip!==undefined?`${Number(x.cost_per_trip).toFixed(2)} zł`:"—"}</td><td>{x.active===false?"Nieaktywny":"Aktywny"}</td><td><button className="btn secondary company-small-btn" onClick={()=>edit(x)}>EDYTUJ</button><PortalAccessButton type="driver" id={x.id} active={!!x.user_id}/></td></tr>)}</tbody></table>
    </div>
  </>;
}
