...
export default function CompanyCreateForm(){
  ...
  const [f,setF]=useState({name:"",nip:"",email:"",phone:"",contactPerson:"",paymentDays:14,discount:0,freeKm:40,defaultPayment:"company_transfer",notes:""});
  async function save(){if(!f.name){setMessage("Podaj nazwę firmy.");return}setSaving(true);
    ...
    if(!r.ok){setMessage(d.error??"Błąd zapisu.");setSaving(false);return}window.location.href=`/panel/firmy/${d.id}`;
  }
  return <div className="company-create-wrap"><button className="btn" onClick={()=>setOpen(!open)}>+ DODAJ FIRMĘ</button>
    {open&&<div className="card company-create-form"><h2>Nowa firma</h2><div className="grid">
      ...
      <label>Rabat %<input type="number" step="0.1" value={f.discount} onChange={e=>setF({...f,discount:Number(e.target.value)})}/></label><label>Darmowy dojazd (km)<input type="number" value={f.freeKm} onChange={e=>setF({...f,freeKm:Number(e.target.value)})}/></label>
    ...
    </div><label style={{marginTop:12}}>Notatki wewnętrzne<textarea rows={3} value={f.notes} onChange={e=>setF({...f,notes:e.target.value})}/></label>
    {message&&<div className="booking-error">{message}</div>}<button className="btn" onClick={save} disabled={saving}>{saving?"ZAPISYWANIE...":"UTWÓRZ FIRMĘ"}</button></div>}
  </div>
}
