export default function B2BPricingSnapshotCard({ booking }: { booking: any }) {
  ...
  const vat = Number(
    booking.vat_price ?? Math.round(net * (vatRate / 100) * 100) / 100
  );
  const gross = Number(booking.price_gross ?? net + vat);
  const distance = Number(booking.pricing_distance_km ?? booking.distance_km ?? 0);
  const freeKm = Number(booking.pricing_free_km ?? 0);
  const billable = Number(
    booking.pricing_billable_km ?? Math.max(0, distance - freeKm)
  );
  const rate = Number(booking.pricing_extra_km_rate_net ?? 0);
  ...
  return (
    <div className="card" style={{ marginTop: 16 }}>
      ...
      <div className="detail-list">
        <div><span>Cennik</span><strong>{booking.pricing_source === "custom" ? "Indywidualny" : booking.pricing_source === "legacy_b2b" ? "Historyczny" : "Standardowy MATT / fallback"}</strong></div>
        ...
        <div><span>Cena bazowa</span><strong>{Number(booking.base_price ?? 0).toFixed(2)} zł netto</strong></div>
        <div><span>Dopłata za km</span><strong>{Number(booking.extra_price ?? 0).toFixed(2)} zł netto</strong></div>
      ...
      <p className="muted" style={{ marginTop: 12 }}>
        Cena została zapisana wraz z warunkami użytymi w chwili wyceny. Późniejsza zmiana cennika firmy nie zmienia tej rezerwacji.
      </p>
    </div>
  );
}
